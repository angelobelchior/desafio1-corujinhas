...
namespace RestauranteSaborDoBrasil.Application.UseCases.Cardapios.Handler
{
    public class BuscarCardapioPorDiaUseCase : UseCaseValidationBase<BuscarCardapioDiaRequest, Cardapio, CardapioResponse>
    {
        ...
        {
            var cardapios = await _baseRepository.GetAllQueryNoTracking.Where(c => (int)c.DiaSemana == (int)DateTime.UtcNow.DayOfWeek
                                                                                && c.Pratos.Any(p => p.Prato.PossuiIngredientes())).ToListAsync();
            return _mapper.Map<CardapioResponse>(cardapios);
        }
    }
}
